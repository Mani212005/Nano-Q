...
app.post("/api/v1/visualize-csv", (req, res) => {
  ...
  const pythonProcess = spawn('python3', ['gemini_viz.py', csv_content, "--csv"]);
...
app.post("/api/v1/ask", (req, res) => {
  ...
  const { csv_content, question } = req.body;

  if (!csv_content || !question) {
    return res.status(400).json({ error: "'csv_content' and 'question' fields are required in the request body." });
  }

  // Call the Python script
  const pythonProcess = spawn('python3', ['gemini_qa.py', csv_content, question, "--csv"]);
